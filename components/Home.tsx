...
import { Building2, Home as HomeIcon, CheckCircle2, AlertCircle, Clock, PieChart, Filter } from 'lucide-react';
...
const isCoordinador = currentUser?.tipo_usuario === 'COORDINADOR';
  ...
  const disponibles = filteredProperties.filter(p => (p.estado || '').toUpperCase() === 'DISPONIBLE').length;
  const apartados = filteredProperties.filter(p => (p.estado || '').toUpperCase() === 'APARTADO');
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500 pb-10">
      ...
      {/* HEADER PRINCIPAL */}
      <div className="bg-white dark:bg-slate-800 p-6 rounded-2xl shadow-md border border-slate-200 dark:border-slate-700 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 transition-colors duration-200">
        <div>
        ...
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          <div className="relative flex items-center">
            ...
            <select 
              ...
              className="pl-9 pr-4 py-2.5 bg-slate-50 dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-xl text-sm font-bold text-slate-700 dark:text-slate-300 outline-none focus:ring-2 focus:ring-indigo-500 w-full md:w-48 appearance-none transition-colors"
            >
              <option value="">TODOS LOS DESARROLLOS</option>
              {desarrollosDisponibles.map(d => <option key={d} value={String(d)}>{d}</option>)}
            </select>
          </div>
          <div className="bg-slate-100 dark:bg-slate-900/50 text-slate-600 dark:text-slate-300 px-4 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest border border-slate-200 dark:border-slate-700 shadow-sm transition-colors flex items-center justify-center">
          ...
          <div className="p-6 border-b border-slate-200 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/20 shrink-0">
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <button 
                ...
                className="bg-indigo-100 dark:bg-indigo-900/40 text-indigo-800 dark:text-indigo-300 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest border border-indigo-200 dark:border-indigo-800 shadow-sm flex items-center gap-2 hover:bg-indigo-200 dark:hover:bg-indigo-800/60 transition-colors cursor-pointer"
              >
